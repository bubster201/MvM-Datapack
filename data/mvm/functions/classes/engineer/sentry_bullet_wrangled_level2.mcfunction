...
execute as @s[scores={mvm_distance=..100}] unless entity @e[type=marker,name="mvm_Hitbox",distance=..0.3] positioned ^ ^ ^0.2 if block ~ ~ ~ minecraft:air run function mvm:classes/engineer/sentry_bullet_wrangled_level2
...
scoreboard players add @e[type=marker,name="mvm_Hitbox",distance=..0.3,limit=1,sort=nearest] mvm_applyDamage 32