execute as @e[type=armor_stand,tag=mvm_building,limit=1,sort=nearest,distance=..2] run scoreboard players operation @s mvm_metal += @s mvm_metalReference
execute as @e[type=armor_stand,tag=mvm_building,limit=1,sort=nearest,distance=..2] if score @s mvm_metal > $200 mvm_vars run scoreboard players set @s mvm_metal 200
